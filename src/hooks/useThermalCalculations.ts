...
interface ThermalCalculationHook {
  results: ThermalResult;
  errors: string[];
  calculate: (inputs: ThermalInput) => void;
  clear: () => void;
}

export function useThermalCalculations(): ThermalCalculationHook {
  const [results, setResults] = useState<ThermalResult>({});
